...
class InstagramIndexView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        suggested_user_list = get_user_model().objects.all().exclude(pk=self.request.user.pk)\
...
class UserPage(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        queryset = self.model.objects.filter(username=self.kwargs.get("username")).first()
        return queryset
...
class UserFollowView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        follow_user = get_object_or_404(get_user_model(), username=self.kwargs.get("username"))
...
class UserUnFollowView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        unfollow_user = get_object_or_404(get_user_model(), username=self.kwargs.get("username"))
